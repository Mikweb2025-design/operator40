...
> **Read this to understand everything in 5 minutes.** Updated **2026-08-24** — `main` @ `5898c39` (index-CF_01HtD.js / o40-v434931f7) + `deploy-tmp` `deffecb`. Deterministic build `2.0.0 · <git-short>`.
...
Dark military UI (`INK`/`OLIVE`/`BLAZE`/`KHAKI`/`PAPER`), 18 programs (A–M + N,O,P + Q), 20+ exercises with MP4/WebP local, streak/goal/medals, NEFFEX music offline, Apple Health import, **PWA push (works with app closed)**, **Belly 2.0 test**, **Pose real-time**.
...
- **Tech:** React 18, Vite 5, Capacitor 6, `idb` 8, `@mediapipe/tasks-vision` 0.10, lucide-react, recharts, Playwright, PHP 8.3 `web-push` 9.0
...
│   ├── api/                # NEW push backend (VAPID) — push-subscribe/unsubscribe/update-stats/send/cron + vapid.json + subscriptions.json/push-stats.json (gitignored)
│   ├── manifest.webmanifest
│   └── sw.js               # offline + push (push/notificationclick) + precache 6 critical clips, __VERSION__ → o40-v<hash>
├── src/
│   ├── App.jsx             # ★ ENTIRE APP (~3300 lines) — see §3
│   ├── main.jsx            # bootstrap React + window.storage (IDB/Preferences) + PWA install + SW reload defer
│   ├── storage.js          # NEW IndexedDB (idb) for web + Capacitor Preferences for native, migration from localStorage
...
│   ├── data/
│   │   ├── exercises.js    # EXERCISES { squat, flessioni/pushup, deadbug… } + EXERCISE_GROUPS
...
│   │   ├── motivation.js   # NEW getMotivationalMessage 6 types (comeback, streak, risk, start, stress, generic) per it/en/de + personalize(name), buildPushPayload
│   │   ├── push.js         # NEW VAPID, subscribePush, unsubscribePush, updatePushStats, testPushViaSW(lang), isPushSupported
│   │   ├── belly.js        # isBellyProgram, getBellySessions, getBellyCount, getBellyStreak, getBellyProgress, pickBellyNext, getBellyInsight
│   │   ├── bellyTest.js    # NEW BELLY_LEVELS, getBellyLevelForTest({plankSec,crunchReps}), shouldProgressBellyLevel 7d
...
│   │   ├── BellyTest.jsx   # NEW plank timer + crunch 30s counter → auto level
│   │   ├── BeforeAfterSlider.jsx # NEW before/after photo slider
│   │   ├── PoseCounter.jsx # NEW MediaPipe Pose real-time squat/push-up counter, offline
