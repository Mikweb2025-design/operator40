...
export function getSmartInsight({ sessions, profile, waistHistory, weightHistory, lang = 'it' }) {
  ...
  const streak = (()=>{ const s=new Set(sessions.map(v=>v.date.slice(0,10))); let cur=new Date(); if(!s.has(cur.toISOString().slice(0,10))) cur.setDate(cur.getDate()-1); let n=0; while(s.has(cur.toISOString().slice(0,10))){n++; cur.setDate(cur.getDate()-1);} return n; })();
  ...
  return { icon: '💡', title: lang === 'it' ? 'Continua così' : 'Keep going', body: lang === 'it' ? `Hai ${n} sessioni, streak ${streak}. Prossimo livello: ${getRank(n).next ? getRank(n).next.min - n + ' sessioni' : 'veterano!'}` : `You have ${n} sessions, streak ${streak}.`, color: '#B8AE8C' };
}
