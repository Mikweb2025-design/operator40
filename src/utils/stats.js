...
export function hrZone(bpm, age, lang) {
  ...
  if (pct < 60)
    return { label: tr({ it: 'Recupero', en: 'Recovery', de: 'Erholung' }, lang), color: STEEL };
  if (pct < 70)
    return {
      label: tr({ it: 'Brucia grassi', en: 'Fat burn', de: 'Fett verbrennen' }, lang),
      color: OLIVE,
    };
  if (pct < 85)
    return { label: tr({ it: 'Cardio', en: 'Cardio', de: 'Cardio' }, lang), color: BLAZE };
...
export const MEDAL_DEFS = {
  ...
  kcal: KCAL_BADGES.map((n) => ({
    type: 'kcal',
    n,
    label: `${n >= 1000 ? n / 1000 + 'k' : n} kcal`,
    icon: '🔥',
    color: '#E84B2A',
  })),
  consistency: CONSISTENCY_BADGES.map((n) => ({
    type: 'consistency',
    n,
    label: `${n}% costanza`,
    icon: '◎',
  ...
  perfect: PERFECT_WEEK_BADGES.map((n) => ({
    type: 'perfect',
    n,
    label: `${n} sett. perfette`,
    icon: '★',
    color: '#D9B34C',
  })),
};
...
export function nextBadge(sessions) {
  ...
  const candidates = [
    ...STREAK_BADGES.filter((n) => bestStreak < n).map((n) => ({
      kind: 'serie',
      n,
      remaining: n - bestStreak,
    })),
    ...SESSION_BADGES.filter((n) => sessions.length < n).map((n) => ({
      kind: 'sessioni',
      n,
      remaining: n - sessions.length,
    })),
  ];
...
export function getMedalProgress(sessions) {
  ...
  for (let w = 0; w < 12; w++) {
    ...
    if (done >= WEEKLY_GOAL) perfectWeeks++;
      ...
      /* settimana in corso non conta come break */
    }
  }

  const all = [];
  for (const n of STREAK_BADGES)
    all.push({
      type: 'streak',
      n,
      label: `${n}gg serie`,
      ...
      color: '#C1440E',
      value: bestStreak,
      unlocked: bestStreak >= n,
      progress: Math.min(1, bestStreak / n),
    });
  for (const n of SESSION_BADGES)
    all.push({
      type: 'sessions',
      n,
      label: `${n} sessioni`,
      ...
      color: '#B8AE8C',
      value: totalSessions,
      unlocked: totalSessions >= n,
      progress: Math.min(1, totalSessions / n),
    });
  for (const n of KCAL_BADGES)
    all.push({
      type: 'kcal',
      n,
      label: `${n >= 1000 ? n / 1000 + 'k' : n} kcal`,
      ...
      color: '#E84B2A',
      value: totalKcal,
      unlocked: totalKcal >= n,
      progress: Math.min(1, totalKcal / n),
    });
  for (const n of CONSISTENCY_BADGES)
    all.push({
      type: 'consistency',
      n,
      label: `${n}% costanza`,
      ...
      value: cons,
      unlocked: cons >= n,
      progress: Math.min(1, cons / n),
    });
  for (const n of PERFECT_WEEK_BADGES)
    all.push({
      type: 'perfect',
      n,
      label: `${n} sett. perfette`,
      ...
      color: '#D9B34C',
      value: perfectWeeks,
      unlocked: perfectWeeks >= n,
      progress: Math.min(1, perfectWeeks / n),
    });
  return {
    all,
    unlocked: all.filter((m) => m.unlocked),
    locked: all.filter((m) => !m.unlocked),
...
export function greeting(lang) {
  ...
  if (h < 18)
    return tr({ it: 'Buon pomeriggio,', en: 'Good afternoon,', de: 'Guten Nachmittag,' }, lang);
