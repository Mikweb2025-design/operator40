...
export function getDailyInsight({ sessions, profile, waistHistory, weightHistory, lang='it' }) {
  ...
  if (avgDone >= (profile?.weeklyGoal || 3)) return { icon: '🚀', title: 'In forma', body: lang==='it' ? `Media ${avgDone.toFixed(1)}/sett. — alza a ${(profile?.weeklyGoal||3)+1} se vuoi spingere.` : `Avg ${avgDone.toFixed(1)}/week — raise to ${(profile?.weeklyGoal||3)+1} to push.`, color: '#7FB069', tip: 'Prova Potenza Esplosiva (L).' };
