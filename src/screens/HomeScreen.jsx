    ...
    <div className="o40-screen-in" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      {showPinnedProgress && (
        <div style={{ margin: '12px 16px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {isRisk && (
            <div style={{ background: `linear-gradient(135deg, ${risk === 'break' ? BLAZE : KHAKI}18, ${INK_2})`, border: `1px solid ${risk === 'break' ? BLAZE : KHAKI}`, borderRadius: 12, padding: '11px 13px', display: 'flex', alignItems: 'center', gap: 11 }}>
              <div style={{ width: 34, height: 34, borderRadius: '50%', background: `${risk === 'break' ? BLAZE : KHAKI}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          {canPromote && nextLevel && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: `linear-gradient(135deg, ${BLAZE_DEEP}, ${INK_2})`, border: `1px solid ${BLAZE}`, borderRadius: 12, padding: '11px 13px' }}>
              ...
              <button onClick={onPromote} style={{ background: BLAZE, border: 'none', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', flexShrink: 0 }}>
      ...
      <div style={{ margin: '12px 16px 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ background: `linear-gradient(135deg, ${INK_2}, ${OLIVE_DARK})`, border: `1px solid ${OLIVE}`, borderRadius: 12, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <CollapsibleSection id="progressi" title="PROGRESSI" icon={TrendingUp} badge={`${cons}% · ${wp.done}/${wp.total} · ${streak}gg`} defaultOpen={showPinnedProgress}>
        ...
        {!showPinnedProgress && (
          <div style={{ marginBottom: 12, background: `linear-gradient(135deg, ${INK_2}, ${OLIVE_DARK})`, border: `1px solid ${OLIVE}`, borderRadius: 12, padding: '11px 13px', display: 'flex', alignItems: 'center', gap: 11 }}>
        ...
        <div style={{ marginBottom: 12, background: `linear-gradient(135deg, ${wp.isDone ? '#7FB069' : BLAZE}18, ${INK_2})`, border: `1px solid ${wp.isDone ? '#7FB069' : BLAZE}55`, borderRadius: 12, padding: '11px 13px', display: 'flex', gap: 11, alignItems: 'center', opacity: showPinnedProgress ? 0.6 : 1 }}>
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: `${wp.isDone ? '#7FB069' : BLAZE}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 18 }}>{wp.isDone ? '✓' : '◐'}</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: PAPER, fontSize: 12.5, fontWeight: 600 }}>{wp.isDone ? 'Obiettivo raggiunto!' : `${wp.remain} ${wp.remain === 1 ? 'sessione' : 'sessioni'} alla meta`} <span style={{ color: STEEL, fontWeight: 400 }}>· ~{estimateWeeklyCalories(sessions, weeklyGoal)} kcal/sett.</span></div>
            <div style={{ marginTop: 6 }}><MiniGoalBar history={getGoalHistory(sessions, weeklyGoal, 6)} /></div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
        ...
        {(() => {
          ...
          return (
            <div style={{ marginBottom: 12, background: `linear-gradient(135deg, ${smart.color}18, ${INK_2})`, border: `1px solid ${smart.color}55`, borderRadius: 12, padding: '11px 13px', display: 'flex', gap: 11, alignItems: 'center' }}>
        ...
        {(() => {
          ...
          return (
            <div style={{ background: `linear-gradient(135deg, ${INK_2}, ${OLIVE_DARK})`, border: `1px solid ${ch.color}55`, borderRadius: 12, padding: '11px 13px' }}>
      ...
      <CollapsibleSection id="missioni" title="MISSIONI & OBIETTIVI" icon={Target} badge={`${bellyProgress.done}/${bellyProgress.total} · ${customPrograms.length} custom`} defaultOpen={false}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {getBellyMissions({ sessions, profile, waistHistory }).map(p => (
              <button key={p.id} onClick={() => onOpenProgram(p)} style={{
                ...
                {getBellyStreak(sessions) >= 2 && <span style={{ position:'absolute', top:4, right:4, background: BLAZE, color: PAPER, fontSize:7, fontWeight:700, borderRadius:4, padding:'1px 4px' }}>🔥{getBellyStreak(sessions)}</span>}
              </button>
            ))}
          </div>
          <div style={{ display:'flex', justifyContent:'space-between', marginTop:8, color: STEEL, fontSize:10 }}>
            <span>Streak pancia: <b style={{color: KHAKI}}>{getBellyStreak(sessions)} gg</b></span>
        ...
        <button onClick={() => setShowOthers(v => !v)} style={{
         display: 'flex', alignItems: 'center', gap: 12, width: '100%',
         background: showOthers ? OLIVE_DARK : INK_2, border: `1px solid ${showOthers ? BLAZE : OLIVE}`, borderRadius: 12, padding: '12px 14px', cursor: 'pointer', marginBottom: 12,
       ...
       {showOthers && (
        ...
        <div className="o40-mono" style={{ color: KHAKI, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '12px 0 8px' }}>{t('home.yours')}</div>
      ...
      <CollapsibleSection id="achievements" title="ACHIEVEMENTS" icon={Medal} badge={`${unlockedAch}/${ach.length}`} defaultOpen={false}>
        <button onClick={() => setShowAchievements(true)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, background: `linear-gradient(135deg, ${INK_2}, ${OLIVE_DARK})`, border: `1px solid ${KHAKI}44`, borderRadius: 12, padding: 14, cursor: 'pointer', textAlign: 'left' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="o40-mono" style={{ color: KHAKI, fontSize: 11, letterSpacing: '0.06em' }}>ACHIEVEMENTS • {unlockedAch}/{ach.length}</div>
            <div style={{ color: PAPER, fontSize: 13, fontWeight: 600 }}>{unlockedAch === ach.length ? 'Tutte sbloccate!' : `${ach.length - unlockedAch} mancanti — tocca per vedere`}</div>
            <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
              {ach.slice(0, 4).map(a => (
                <span key={a.id} style={{ fontSize: 14, opacity: a.unlocked ? 1 : 0.4 }}>{a.icon}</span>
              ))}
              <span style={{ color: STEEL, fontSize: 11, marginLeft: 6 }}>+{ach.length - 4} altri</span>
            </div>
          </div>
        ...
        {showAchievements && <AchievementsModal sessions={sessions} onClose={() => setShowAchievements(false)} />}
      </CollapsibleSection>
