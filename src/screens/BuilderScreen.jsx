...
function BuilderScreen({ profile, initial, onCancel, onCreate, onUpdate }) {
  ...
  const kcal = canCreate
    ? Math.round(
        estimateProgramKcal(draft, profile.weight, !!profile.skipWarmup, preset.work, preset.rest)
      )
    : 0;
  const mins = canCreate
    ? Math.round(totalSeqSeconds(draft, !!profile.skipWarmup, preset.work, preset.rest) / 60)
    : 0;
      ...
      <div className="o40-scroll" style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[1, 2, 3].map((r) => (
              ...
              >
                <span className="o40-display" style={{ color: PAPER, fontSize: 18 }}>
                  {r}
                </span>
              </button>
            ))}
        ...
        {selected.length > 0 && (
          ...
          >
            ...
            >
              Ordine selezionati · trascina su/giù
            </div>
            {selected.map((sid, idx) => (
              ...
              >
                ...
                <span style={{ flex: 1, color: PAPER, fontSize: 12 }}>
                  {EXERCISES[sid] ? EXERCISES[sid].name.it : sid}
                </span>
                <button
                  disabled={idx === 0}
                  onClick={() =>
      ...
      <div style={{ padding: 16, borderTop: `1px solid ${OLIVE_DARK}` }}>
        {canCreate ? (
          <>
            ...
            >
              <span>{t('bld.min', { m: mins })}</span>
              <span>·</span>
              <span>{t('bld.kcal', { k: kcal })}</span>
            </div>
            <button onClick={() => (isEdit ? onUpdate(draft) : onCreate(draft))} style={primaryBtn}>
              <Check size={18} /> {isEdit ? 'AGGIORNA' : t('bld.create.go')}
            </button>
          </>
        ) : (
