...
function missionCounts(sessions) {
  ...
  sessions.forEach((s) => {
    if (counts[s.programId] !== undefined) counts[s.programId]++;
  });
  return counts;
}
    ...
    >
      <div
        style={{
          ...
          boxShadow: unlocked
            ? `0 0 12px ${color}66, inset 0 1px 1px rgba(255,255,255,0.15)`
            : 'none',
          position: 'relative',
          overflow: 'hidden',
        }}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {[
        ...
        <div key={cat.key}>
          ...
          >
            ...
            >
              {unlocked.filter((m) => m.type === cat.key).length}/{byType[cat.key].length}
            </span>
          </div>
      ...
      {next.length > 0 && (
        ...
        >
          ...
          <span style={{ color: KHAKI, fontSize: 11, flex: 1 }}>
            Prossime:{' '}
            {next
              .map(
                (m) =>
                  `${m.icon} ${m.n}${m.type === 'kcal' ? '' : m.type === 'streak' ? 'gg' : ''} (${Math.round(m.progress * 100)}%)`
              )
              .join(' • ')}
          </span>
        </div>
...
function HistoryScreen({
  ...
  const now = Date.now();
  const thisWeekKcal = sessions
    .filter((s) => new Date(s.date).getTime() > now - 7 * 86400000)
    .reduce((a, s) => a + s.kcal, 0);
  const lastWeekKcal = sessions
    .filter((s) => {
    ...
    .reduce((a, s) => a + s.kcal, 0);
  const trendPct =
    lastWeekKcal > 0 ? Math.round(((thisWeekKcal - lastWeekKcal) / lastWeekKcal) * 100) : null;
  ...
  const avgRpe = rpeSeries.length
    ? rpeSeries.reduce((a, b) => a + b.rpe, 0) / rpeSeries.length
    : null;
  const bestWeekKcal = (() => {
    let best = 0;
    sessions.forEach((s) => {
    ...
    sessions.length >= 2
      ? sessions.length /
        Math.max(
          1,
          Math.round(
            (new Date(sessions[sessions.length - 1].date) - new Date(sessions[0].date)) /
              (7 * 86400000)
          )
        )
      : sessions.length;
      ...
      <div className="o40-scroll" style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
          <DogTag
            ...
            accent={streak > 0}
          />
          <DogTag label={t('dt.record')} value={bestStreak} sub={t('dt.beststreak')} accent={bestStreak >= 7} />
          <DogTag label={t('dt.kcal')} value={totalKcal} sub={t('dt.total')} accent={totalKcal > 0} />
        </div>

        <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
          <DogTag label={t('dt.minutes')} value={totalMin} sub={t('dt.trained')} />
          <DogTag label={t('dt.avgkcal')} value={avgKcal} sub={t('dt.permission')} />
          <DogTag label={t('dt.weeks')} value={sessionsPerWeek.toFixed(1)} sub={t('dt.perweek')} accent={sessionsPerWeek >= 2} />
        </div>
        ...
        {pr && (
          ...
          >
            ...
            <div style={{ width: 1, background: `linear-gradient(180deg, transparent, ${OLIVE}55, transparent)` }} />
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}><Crown size={14} color={KHAKI} /></div>
              <div className="o40-mono" style={{ color: KHAKI, fontSize: 9, letterSpacing: '0.07em' }}>
                MAX STREAK
              </div>
              <div className="o40-display o40-num-glow" style={{ fontSize: 22, lineHeight: 1.1 }}>
                {pr.maxStreak}🔥
              </div>
            </div>
            <div style={{ width: 1, background: `linear-gradient(180deg, transparent, ${OLIVE}55, transparent)` }} />
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}><Ruler size={14} color={STEEL} /></div>
              <div className="o40-mono" style={{ color: KHAKI, fontSize: 9, letterSpacing: '0.07em' }}>
                TOTALE
              </div>
              <div className="o40-display o40-num-glow" style={{ fontSize: 22, lineHeight: 1.1 }}>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: PAPER, fontSize: 13, fontWeight: 600 }}>
                {t('hist.bestweek.title')}
              </div>
              <div style={{ color: STEEL, fontSize: 11.5 }}>{t('hist.bestweek.sub')}</div>
            </div>
            <span className="o40-display" style={{ color: BLAZE, fontSize: 22 }}>
              {bestWeekKcal}
            </span>
        ...
        {(() => {
          ...
          return (
            ...
            >
              ...
              >
                <Zap size={12} color={BLAZE} /> ADERENZA 8 SETTIMANE
              </div>
              <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                ...
                >
                  ...
                  >
                    {risk === 'ok' ? '● ok' : risk === 'at-risk' ? '◐ a rischio' : '○ break'}
                  </div>
                </div>
                <div
                  style={{
              ...
              >
                ...
                <span style={{ color: wp.isDone ? '#7FB069' : KHAKI }}>
                  {wp.isDone ? 'Completata!' : `${wp.remain} mancanti`}
                </span>
              </div>
            </div>
          );
            ...
            <div style={{ marginBottom: 18, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ color: PAPER, fontSize: 13, fontWeight: 600 }}>
                    {daily.title}{' '}
                    <span style={{ color: STEEL, fontSize: 11, fontWeight: 400 }}>
                      · {daily.tip}
                    </span>
                  </div>
                  <div style={{ color: STEEL, fontSize: 11.5, marginTop: 2, lineHeight: 1.4 }}>
                    {daily.body}
                  </div>
                </div>
              ...
              >
                ...
                <div style={{ color: weekly.color, fontSize: 18, fontWeight: 700 }}>
                  {weekly.body.split('/')[0]?.trim()}
                </div>
              </div>
            </div>
          );
            ...
            {(() => {
              ...
              return (
                <div>
                  ...
                  >
                    <span>
                      8 sett. · {streakW} {streakW === 1 ? 'settimana' : 'settimane'} streak
                    </span>
                    {sugg !== weeklyGoal && (
                      <span style={{ color: KHAKI }}>
                        <Lightbulb size={10} style={{ verticalAlign: 'middle', marginRight: 4 }} />
                        Suggerito: {formatGoal(sugg)}
                      </span>
                    )}
                  </div>
                  <div style={{ color: STEEL, fontSize: 10.5, marginTop: 4 }}>
                    ~{estimateWeeklyCalories(sessions, weeklyGoal)} kcal/sett. a goal {weeklyGoal}
                  </div>
                </div>
              );
            })()}
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          >
            <Medal size={12} color={KHAKI} /> {t('hist.milestones')}{' '}
            <span style={{ color: STEEL, fontSize: 10, marginLeft: 6 }}>
              {getMedalProgress(sessions).unlocked.length}/{getMedalProgress(sessions).all.length}{' '}
              sbloccate
            </span>
          </div>
          <div
              ...
              <BarChart data={weekData} margin={{ top: 8, right: 8, left: -22, bottom: 0 }}>
                <defs>
                  <linearGradient id="kcalGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={BLAZE} stopOpacity={0.95} />
                    <stop offset="100%" stopColor={BLAZE_DEEP} stopOpacity={0.85} />
                  </linearGradient>
                </defs>
                <CartesianGrid stroke={OLIVE_DARK} strokeDasharray="3 3" vertical={false} opacity={0.35} />
                <XAxis
                  dataKey="label"
                ...
                />
                <Bar dataKey="kcal" fill="url(#kcalGrad)" radius={[6, 6, 0, 0]} stroke={BLAZE} strokeOpacity={0.35} strokeWidth={1} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              {PROGRAMS.map((p) => (
                <div key={p.id}>
                  ...
                  >
                    ...
                    <span className="o40-mono" style={{ color: STEEL }}>
                      {counts[p.id]}
                    </span>
                  </div>
                  <div
        ...
        {hrData.length >= 2 && (
          <div style={{ marginBottom: 20 }}>
            ...
            >
              <HeartPulse size={12} color={BLAZE} /> {t('hist.hr')}
            </div>
            <div
              style={{
                background: `linear-gradient(135deg, ${INK_2} 0%, ${INK} 100%)`,
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={hrData} margin={{ top: 8, right: 12, left: -18, bottom: 0 }}>
                  <CartesianGrid stroke={OLIVE_DARK} strokeDasharray="3 3" opacity={0.3} />
                  <XAxis
                    dataKey="label"
                    tick={{ fill: STEEL, fontSize: 10, fontWeight: 500 }}
                    axisLine={{ stroke: OLIVE, opacity: 0.3 }}
        ...
        {waistData.length >= 2 && (
          <div style={{ marginBottom: 20 }}>
            ...
            >
              ...
              >
                <Ruler size={12} color={KHAKI} /> {t('hist.waist')}
              </div>
              <span
                className="o40-mono"
                style={{
              ...
              >
                {t('hist.waist.total', {
                  v:
                    (waistData[waistData.length - 1].cm - waistData[0].cm > 0 ? '+' : '') +
                    (waistData[waistData.length - 1].cm - waistData[0].cm),
                })}
              </span>
            </div>
            <div
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              ...
              <span
                ...
                style={{
                  color:
                    weightData[weightData.length - 1].kg <= weightData[0].kg ? '#7FB069' : BLAZE,
                  fontSize: 11,
                }}
              >
                {t('hist.weight.total', {
                  v:
                    (weightData[weightData.length - 1].kg - weightData[0].kg > 0 ? '+' : '') +
        ...
        {profile &&
          ...
          (() => {
            ...
            const waistLatest = waistHistory.length
              ? waistHistory[waistHistory.length - 1].cm
              : null;
            ...
            return (
              ...
              >
                ...
                >
                  {t('bmi.title')} · {bmi}{' '}
                  {cat && <span style={{ color: cat.color }}>· {t('bmi.' + cat.key)}</span>}
                </div>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 6 }}>
                  {tdee && (
                    <span style={{ color: STEEL, fontSize: 12 }}>{t('bmi.tdee', { v: tdee })}</span>
                  )}
                  {bf != null && <span style={{ color: KHAKI, fontSize: 12 }}>· BF {bf}%</span>}
                  {wht != null && (
                    <span style={{ color: wcat.color, fontSize: 12 }}>· WHtR {wht.toFixed(2)}</span>
                  )}
                </div>
                <div style={{ color: STEEL, fontSize: 11.5, marginTop: 4, opacity: 0.8 }}>
                  {tr(simpleMealHint(bmi > 27 ? 'cut' : 'maintain'), lang)}
                </div>
              </div>
            );
          })()}
        {(() => {
          ...
          return (
            ...
            >
              ...
              >
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: BLAZE, boxShadow: `0 0 6px ${BLAZE}66` }} /> {t('export.calendar')} ·{' '}
              ...
              >
                {['L', 'M', 'M', 'G', 'V', 'S', 'D'].map((d) => (
                  <div key={d} style={{ color: STEEL, fontSize: 10 }}>
                    {d}
                  </div>
                ))}
        ...
        >
          ...
          {photos.length === 0 ? (
            <div style={{ color: STEEL, fontSize: 12 }}>
              Nessuna foto — aggiungi la prima per vedere il prima/dopo
            </div>
          ) : (
            <>
        ...
        >
          {t('hist.sessions.title')}
        </div>
        {ordered.length === 0 && (
          <div style={{ color: STEEL, fontSize: 13 }}>{t('hist.empty')}</div>
        )}
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: PAPER, fontSize: 14, fontWeight: 600 }}>
                      {s.programName}
                    </div>
                    <div style={{ color: STEEL, fontSize: 11.5 }}>
                      {new Date(s.date).toLocaleDateString(LOCALES[lang], {
                        weekday: 'short',
                        day: '2-digit',
                        month: 'short',
                      })}
                  ...
                  >
                    <Flame size={13} color={BLAZE} /> {s.kcal}
                  </div>
                  {zone && (
                    <div
                      style={{
                  ...
                  >
                    {confirmDeleteDate === s.date ? (
                      <Check size={14} color={BLAZE} />
                    ) : (
                      <X size={14} color={STEEL} />
                    )}
                  </button>
                </div>
                {s.notes && (
      ...
      {confirmClear && (
        ...
        >
          ...
          >
            ...
            <div style={{ color: STEEL, fontSize: 13, marginBottom: 18 }}>
              {t('hist.clear.body')}
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setConfirmClear(false)} style={{ ...secondaryBtn, flex: 1 }}>
                {t('hist.clear.cancel')}
              </button>
              <button
                onClick={() => {
