...
export class WallsitAnalyzer extends ExerciseAnalyzer{
  ...
  analyze(lm: PoseLandmarks, ts:number, _dt:number, q:PoseQualityResult){
    ...
    const _temporal = this.getTemporalClassifier('wallsit');
    ...
    return { phase:this.phase, enginePhase:'ready' as any, repIncrement:false, repConfidence:0, formScore: clamp(form,0,100), poseQuality:q, cues, primaryAngle:k, secondaryAngles:{ temporalROM: Math.round(this.temporalBuffer.getROM('kneeRaw')),  trunk }, velocity:0, direction:'hold' as any };
  }
  reset(){ super.reset(); }
}
