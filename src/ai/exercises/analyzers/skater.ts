...
export class SkaterAnalyzer extends ExerciseAnalyzer{
  ...
  analyze(lm: PoseLandmarks, ts:number, dtMs:number, q:PoseQualityResult){
    ...
    const _temporal = this.getTemporalClassifier('skater');
    ...
    this.lastX=cx;
    return { phase:this.phase, enginePhase: this.phase==='LANDED'?'bottom':'ready' as any, repIncrement: repInc, repConfidence: repConf, formScore: clamp(88,0,100), poseQuality:q, cues:[], primaryAngle: spread*100, secondaryAngles:{ temporalROM: Math.round(this.temporalBuffer.getROM('kneeRaw')),  knee, spread, velX: this.velX }, velocity:this.velX*100, direction: Math.abs(this.velX)<0.2?'hold': this.velX>0?'up':'down' as any };
  }
  reset(){ super.reset(); }
}
