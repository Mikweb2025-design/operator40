...
export function WeeklyChallenge({ sessions = [], weeklyGoal = 3 }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div className="o40-mono" style={{ color: KHAKI, fontSize: 10, letterSpacing: '0.07em' }}>
          SFIDA SETTIMANALE
        </div>
        <div style={{ color: PAPER, fontWeight: 800, fontSize: 14 }}>
          {isDone ? 'Completata!' : `${done}/${weeklyGoal} missioni`}
        </div>
        <div style={{ color: STEEL, fontSize: 12 }}>
          {isDone
            ? 'Obiettivo raggiunto, mantieni il fuoco'
            : `Mancano ${remain} per chiudere la settimana`}
        </div>
      </div>
      <div style={{ textAlign: 'right' }}>
        <div className="o40-display" style={{ color: BLAZE, fontSize: 22 }}>
          {Math.round(pct * 100)}%
        </div>
        <div
          style={{
