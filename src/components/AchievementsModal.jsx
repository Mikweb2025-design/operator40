...
function Badge({ label, unlocked, value, color = BLAZE, icon = 'trophy', progress = 1 }) {
  ...
  function StarFallback(props) {
    return <span style={{ fontSize: 16 }}>{unlocked ? '★' : '☆'}</span>;
  }
  ...
  return (
    ...
    >
      ...
      {!unlocked && (
        <div style={{ width: '100%', height: 3, borderRadius: 2, background: OLIVE, marginTop: 2 }}>
          <div
            style={{ width: `${Math.round(progress * 100)}%`, height: '100%', background: color }}
          />
        </div>
      )}
    </div>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            >
              ACHIEVEMENTS • {unlocked}/{ach.length}
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            {ach.map((a) => (
              ...
              >
                ...
                >
                  {a.unlocked ? 'Sbloccato' : `${Math.round(a.progress * 100)}%`}
                </div>
                <div style={{ height: 3, borderRadius: 2, background: OLIVE, marginTop: 6 }}>
                  <div
                    style={{
          ...
          {nextAch.length > 0 && (
            ...
            >
              ...
              >
                <Sparkles size={12} /> PROSSIMI • {nextAch.length}
              </div>
          ...
          >
            ...
            >
              MEDAGLIE • {medals.unlocked.length}/{medals.all.length}
            </div>
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto' }}>
              {medals.all.slice(0, 8).map((m) => (
                ...
                >
                  ...
                  >
                    <Medal size={14} color={m.unlocked ? PAPER : STEEL} />
                  </div>
                  <div style={{ color: m.unlocked ? PAPER : STEEL, fontSize: 9, marginTop: 4 }}>
                    {m.n}
                  </div>
                </div>
              ))}
