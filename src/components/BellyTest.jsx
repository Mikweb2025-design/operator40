...
export default function BellyTest({ onSave, onClose, lang = 'it', initial = null }) {
  ...
  const [crunchReps, setCrunchReps] = useState(
    initial?.crunchReps ? String(initial.crunchReps) : ''
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        ...
        >
          <div className="o40-mono" style={{ color: KHAKI, fontSize: 10, marginBottom: 6 }}>
            PLANK MAX (sec)
          </div>
          <input
            value={plankSec}
          ...
          <div style={{ color: STEEL, fontSize: 9, marginTop: 4 }}>
            &lt;30 Recluta · 30-60 Combattente · &gt;60 Elite
          </div>
        </div>
        ...
        >
          ...
          <div style={{ color: STEEL, fontSize: 9, marginTop: 4 }}>
            &lt;10 Recluta · 10-20 Combattente · &gt;20 Elite
          </div>
        </div>
      </div>
      ...
      {suggested && (
        ...
        >
          ...
          >
            <Trophy size={16} color={BLAZE} />
          </div>
          <div style={{ flex: 1 }}>
            <div className="o40-mono" style={{ color: KHAKI, fontSize: 10 }}>
              LIVELLO SUGGERITO
            </div>
            <div style={{ color: PAPER, fontSize: 14, fontWeight: 700 }}>
              {suggested.label.it} · {suggested.work}″/{suggested.rest}″
            </div>
            <div style={{ color: STEEL, fontSize: 11 }}>{suggested.desc.it}</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div className="o40-mono" style={{ color: BLAZE, fontSize: 10 }}>
              {p}″ / {c} rep
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        >
          <Check size={14} /> {t('Salva livello', 'Save level', 'Level speichern')}{' '}
      ...
      <div style={{ display: 'flex', gap: 4, marginTop: 8, justifyContent: 'center' }}>
        {BELLY_LEVELS.map((l) => (
          <span
            ...
            style={{
              fontSize: 9,
              color: suggested?.key === l.key ? BLAZE : STEEL,
              border: `1px solid ${suggested?.key === l.key ? BLAZE : OLIVE}`,
              borderRadius: 6,
              padding: '2px 6px',
              background: suggested?.key === l.key ? `${BLAZE}18` : 'transparent',
            }}
          >
            {l.label.it}
          </span>
